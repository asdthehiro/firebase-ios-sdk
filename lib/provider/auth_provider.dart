...
class AuthProvider with ChangeNotifier {
  ...
  Future register(RegisterModel registerModel, Function callback) async {
    _isLoading = true;
    notifyListeners();
    try {
      ApiResponse apiResponse = await authRepo!.registration(registerModel);
      _isLoading = false;
      // log("apiResponse.response!.data ${apiResponse.response!.data}");
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        if (apiResponse.response!.data['errNum'] != null &&
            apiResponse.response!.data['errNum'] == 422) {
          callback(
              false, apiResponse.response!.data['errors']['phone'][0] ?? "");
          notifyListeners();
          return;
        }
        ...
        callback(false, apiResponse.response!.data['msg'] ?? "");
        notifyListeners();
      }
    } catch (e) {
  ...
  Future loginPhone(String phone, Function callback) async {
    ...
    try {
      ...
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        clearGuestId();
        String code = apiResponse.response!.data['code'] ?? "";
        callback(true, code, "");
        notifyListeners();
      } else {
        String errorMessage = apiResponse.response!.data['error'] ?? "";
        callback(false, '', errorMessage);
        notifyListeners();
      }
    } catch (e) {
